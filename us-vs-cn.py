...
cn = pd.read_csv("./data/ncp-cn-new.csv", skipinitialspace=True, index_col='Date', parse_dates=True)
...
itn = usn[35:]
cn_start_day = '2020-01-10'
it_start_day = '2020-02-27'

hl = len(cnn)
cnn.index = np.arange(hl-1, -1, -1)

il = len(itn)
itn.index = np.arange(0, il, 1)

xtk = np.arange(0, hl, 5)
cn_day_range = pd.date_range(cn_start_day, periods=hl+5, freq='1d')
xlb = cn_day_range.strftime('%m-%d')
ii = []
for i in xtk:
...
cn_start_day = cn_day_range[0].strftime('%m/%d')
cn_end_day = cn_day_range[il-1].strftime('%m/%d')

#################################
host = host_subplot(111)
par = host.twinx()

plt.title('New confirmed cases of COVID-19 (United States vs. China)\n', {'fontsize':17, 'fontweight':'bold','color': '#0b0c0c', 'verticalalignment':'top'})

#bns=np.arange(-0.5,l+0.5,1)
#par.hist(x, bins=bns, rwidth=0.9, alpha=0.4)
w = 0.5
# C1, C8 #A7C957
par.bar(cnn.index, cnn.values, width=0.5, alpha=0.3, color='#1d70b8', label='China: '+cn_start_day+'~'+cn_end_day)
par.bar(itn.index+w, itn.values, width=w, alpha=1, color='#d5281b', label='   USA: '+it_start_day+'~'+it_end_day)


x = cnn.index.repeat(cnn)
h1 = len(x)**(-1.0/5.0)
...
ofname = "export/us-vs-cn-" + it_day_range[il-1].strftime('%m%d') + '.png'
